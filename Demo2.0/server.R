...
levelTable <- function(data){
  ...
  if(data == "1"){
    ...
    appIdQuery <- paste("Select _id from application_statistics where name =",data,sep = "")
    appId <- dbGetQuery(con, appIdQuery)
    query <- paste("Select timestamp, cpu_usage, other_bytes_sent, other_bytes_received from application_sensor_",idleMessengerId[[1]],sep = "")
    statsData <- dbGetQuery(con, query)  
  }
  statsData
...
shinyServer(
  function(input, output){
    #output$olevel <- renderPrint({menu(input$app)})
    #output$platformTable <- renderPrint(levelTable(input$app))
